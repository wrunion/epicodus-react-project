...
describe('handleClickReducer', () => {
  test('Should return a property called "itemToEdit" whose value is an object, if action type is selectItemToEdit', () => {
    expect(handleClickReducer({}, { type: 'SELECT_ITEM_TO_EDIT', data: {itemName: 'Blue Shirt', price: 'JK Rowling'}})).toEqual({itemToEdit: {itemName: 'Blue Shirt', price: 'JK Rowling'}})
  });

  test('Should return property "selectedItem" whose value is an object, if action type is selectItem', () => {
    expect(handleClickReducer({}, { type: 'SELECT_ITEM', data: {itemName: 'The Hobbit', price: 'JRR Tolkein'}})).toEqual({selectedItem: {itemName: 'The Hobbit', price: 'JRR Tolkein'}});
  });
